  ...
  for(int i = 0; i < K; i++) {
    ...
    for(std::pair<std::string, std::pair<int, int> > iter : updates) {
      ...
      if(temp[iter.second.first] > temp[iter.second.second]) {
        br = ldPair(ldAdd(ldPair(temp[iter.second.second], (distances_1[iter.second.second][i] == 111)), (landmarks.count(iter.second.first) > 0)), e); beta = ldPair(ldPair(temp[iter.second.first], (distances_1[iter.second.first][i] == 111)), true);
        if(br <= beta)
          que.push(std::make_pair(br, iter.second.first));
      } else if(temp[iter.second.first] < temp[iter.second.second]) {
        br = ldPair(ldAdd(ldPair(temp[iter.second.first], (distances_1[iter.second.first][i] == 111)), (landmarks.count(iter.second.second) > 0)), e); beta = ldPair(ldPair(temp[iter.second.second], (distances_1[iter.second.second][i] == 111)), true);
        if(br <= beta)
    ...
    while (!que1.empty() || !que.empty()) {
	  ...
	  br = ldPair(ldAdd(ldDist(p.first), (landmarks.count(w) > 0)), (p.first & 1)==0?true:false); beta = ldPair(ldPair(temp[w], (distances_1[w][i] == 111)), true);
          if(br <= beta)
